...
export default function Logo({ location, className }: LogoProps) {
  if (location === 'header') {
    ...
    return (
      <span
        className={`${fasterOne.className} md:hidden col-span-2 mb-2 text-5xl bg-gradient-to-r from-good-guess from-64% to-wrong-guess to-68% inline-block text-transparent bg-clip-text`}
      >
        <Link href="/">GuessEye</Link>
        <span className="sr-only">Go to Home page</span>
      </span>
